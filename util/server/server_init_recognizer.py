...
def init_recognizer(queue_in: Queue, queue_out: Queue, sockets_id, stdin_fn):
    ...
    try:
        ...
        # 载入语音模型
        console.print('[yellow]语音模型载入中...', end='\n'); t1 = time.time()
        ...
        try:
            if model_type == 'fun_asr_nano':
                logger.info("使用 Fun-ASR-Nano 模型")
                console.print('[cyan]  → 正在初始化 Fun-ASR-Nano 引擎...')
                console.print('[cyan]  → 这可能需要几分钟时间（首次加载或 x86_64 模式下）')
                console.print('[cyan]  → 加载进度：')
                try:
                    recognizer = create_asr_engine(
                        **{key: value for key, value in FunASRNanoGGUFArgs.__dict__.items() if not key.startswith('_')}
                    )
                except Exception as init_error:
                    console.print(f'[red]✗ 模型初始化失败: {init_error}')
                    logger.error(f"Fun-ASR-Nano 初始化失败: {init_error}", exc_info=True)
                    raise
        ...
        logger.error(f"识别子进程发生致命错误: {e}", exc_info=True)
        console.print(f'[red bold]✗ 识别子进程错误: {e}')
        # 通知主进程发生了错误
        try:
            queue_out.put(None)  # 发送 None 表示初始化失败
        except:
            pass
        raise  # 重新抛出异常，让子进程以非0退出码退出
